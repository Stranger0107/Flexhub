...
export default function ProfessorProfile() {
  ...
  const [assignmentsMade, setAssignmentsMade] = useState([]);
  const [loading, setLoading] = useState(true);

  // 🧠 Fetch courses and assignments safely
  useEffect(() => {
    const loadData = async () => {
      try {
        const coursesData = await fetchMyProfessorCourses();
        const assignmentsData = await fetchProfessorAssignments();
        setMyCourses(Array.isArray(coursesData) ? coursesData : []);
        setAssignmentsMade(Array.isArray(assignmentsData) ? assignmentsData : []);
      } catch (err) {
    ...
    if (user && user.role === "professor") loadData();
  }, [user, fetchMyProfessorCourses, fetchProfessorAssignments]);

  if (authLoading || loading) {
    return (
      <div style={{ padding: "2rem", fontSize: "1.3rem" }}>Loading profile...</div>
    );
  }
  ...
  return (
    <div
      style={{
        ...
        minHeight: "100vh",
        background:
          "radial-gradient(circle 600px at 70% 40%, #e0e7ff 0%, #f8fafc 100%)",
      }}
    >
      <h1
        style={{
      ...
      >
        ...
        <div style={{ display: "flex", gap: "1rem", marginBottom: "2.2rem" }}>
          ...
          >
            <FaBook size={28} style={{ marginBottom: 2 }} />
            <br />
            <div style={{ fontSize: "1.6rem", fontWeight: 700 }}>
              {myCourses.length}
            </div>
            <div style={{ fontSize: ".98rem", fontWeight: 300 }}>
              Courses Created
            </div>
          </div>
          <div
          ...
          >
            <FaClipboardList size={25} style={{ marginBottom: 2 }} />
            <br />
            <div style={{ fontSize: "1.6rem", fontWeight: 700 }}>
              {assignmentsMade.length}
            </div>
            <div style={{ fontSize: ".98rem", fontWeight: 300 }}>
              Assignments Given
            </div>
        ...
        >
          ...
          >
            ...
            <span
              style={{ fontWeight: 500, minWidth: 120, color: "#222" }}
            >
              Name:
            </span>
            {editing ? (
